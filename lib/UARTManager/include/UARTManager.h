#ifndef __UART_MANAGER_V2_H
#define __UART_MANAGER_V2_H
...
class UARTManager {
public:
    UARTManager(uart_port_t uart_num = UART, int baud_rate = 115200, int rx_buffer_size = RX_BUF_SIZE * 2, int tx_buffer_size = TX_BUF_SIZE * 2);
    ~UARTManager();
    ...
    bool waitForCommandHex(uint8_t hexCommand, uint8_t time_out_enable = ENABLE_TIME_OUT , uint32_t time_out = TIME_WAIT_RECEIVE_ACCPET_UART);
